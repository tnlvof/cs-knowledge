...
/**
 * T106: 주간 랭킹 API
 * GET /api/ranking/weekly?limit=50
 *
 * 이번 주 획득 EXP(weekly_exp) 순으로 정렬된 랭킹을 반환합니다.
 * weekly_exp > 0인 유저만 포함합니다.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // 주간 랭킹 조회: weekly_exp DESC (weekly_exp > 0만)
    const { data, error } = await sb
      .from("profiles")
      .select("id, nickname, level, exp, job_class, job_tier, avatar_type, supporter_tier, weekly_exp")
      .gt("weekly_exp", 0)
